...
function money(month=0){
  ...
  for (var name in my_works){
    var [_, _, _, sum_money, sum_traffic, sum_time] = my_works[name];
    sum_list["time"] += sum_time;
    sum_list["money"] += sum_money+sum_traffic;
    sum_time = `${parseInt(sum_time)}:${parseInt((sum_time-parseInt(sum_time))*60)}:00`;
    header.push([name, sum_time, sum_money.toLocaleString(), sum_traffic.toLocaleString(), (sum_money + sum_traffic).toLocaleString()]);
  };
